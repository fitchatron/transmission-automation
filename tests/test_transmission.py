...
#      7    90%   689.1 MB  19 sec       0.0  2960.0    0.0  Up & Down    Rick.and.Morty.S09E04.1080p.WEB.h264-EDITH[EZTVx.to].mkv
def test_list_torrents_parses_name_with_multiple_spaces(monkeypatch):
    stdout = "\n".join(
        [
            "    ID   Done       Have  ETA           Up    Down  Ratio  Status       Name",
            "     7     0%       None  Unknown      0.0     0.0   None  Up & Down    Rick.and.Morty.S09E04.1080p.WEB.h264-EDITH[EZTVx.to].mkv",
    ...
    assert torrents[1]["id"] == "10"
    assert torrents[1]["status"] == "Seeding"
    assert (
        torrents[1]["name"]
        == "www.UIndex.org    -    The.Boys.S05E01.1080p.WEB.h264-ETHEL"
    )
...
def test_list_torrents_parses_status_with_spaces(monkeypatch):
    ...
    assert len(torrents) == 4
    assert torrents[2]["id"] == "11"
    assert torrents[2]["status"] == "Up & Down"
    assert (
        torrents[2]["name"] == "The.Boys.S05E02.1080p.HEVC.x265-MeGusta[EZTVx.to].mkv"
    )
